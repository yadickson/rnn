...
class TestMeanSquaredErrorLossFunction(TestCase):
    ...
    def test_should_return_zero_value_when_real_value_and_calculate_value_are_equal(self) -> None:
        value = self.faker.random.randint(10, 20)
        self.assertEqual(0, self.function.value(value, value))

    def test_should_return_mse_value_when_real_value_and_calculate_value_are_not_equal(self) -> None:
    ...
    def test_should_return_zero_value_derived_when_real_value_and_calculate_value_are_equal(self) -> None:
        value = self.faker.random.randint(10, 20)
        self.assertEqual(0, self.function.derived(value, value))

    def test_should_return_mse_value_derived_when_real_value_and_calculate_value_are_not_equal(self) -> None:
        real_value = self.faker.random.randint(17, 19)
        calculated_value = self.faker.random.randint(20, 22)
        self.assertNotEqual(0, self.function.derived(real_value, calculated_value))

    def test_should_return_values_from_array_when_both_are_equals(self) -> None:
    ...
    def test_should_return_values_derived_from_array_when_both_are_equals(self) -> None:
        real_value = np.array([1.8, -1.1, 0.1, -1.0])
        calculated_value = np.array([1.8, -1.1, 0.1, -1.0])
