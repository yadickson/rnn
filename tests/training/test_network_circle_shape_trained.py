...
class TestNetworkCircleShapeTrained(TestCase):
    ...
    def test_should_check_process_true_when_input_is_zero_zero(self) -> None:
        self.assertGreaterEqual(self.network.process([[0, 0]])[-1], [[0.9]])

    def test_should_check_process_true_when_input_is_one_one(self) -> None:
        self.assertGreaterEqual(self.network.process([[1, 1]])[-1], [[0.9]])

    def test_should_check_process_true_when_input_is_minus_one_one(self) -> None:
        self.assertGreaterEqual(self.network.process([[-1, 1]])[-1], [[0.9]])

    def test_should_check_process_true_when_input_is_one_minus_one(self) -> None:
        self.assertGreaterEqual(self.network.process([[1, -1]])[-1], [[0.9]])

    def test_should_check_process_false_when_input_is_two_two(self) -> None:
        self.assertLess(self.network.process([[2, 2]])[-1], [[0.1]])

    def test_should_check_process_false_when_input_is_two_minus_two(self) -> None:
        self.assertLess(self.network.process([[2, -2]])[-1], [[0.1]])

    def test_should_check_process_false_when_input_is_four_four(self) -> None:
        self.assertLess(self.network.process([[4, 4]])[-1], [[0.1]])

    def test_should_check_process_false_when_input_is_minus_four_four(self) -> None:
