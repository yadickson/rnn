...
class TestSigmoidActivationFunction(TestCase):
    ...
    def test_should_return_one_value_when_input_value_is_greater_than_zero(self) -> None:
        input_value = self.faker.random.randint(100, 200)
        self.assertEqual(1.0, self.function.value(input_data=input_value))

    def test_should_return_not_zero_value_when_input_is_one(self) -> None:
        self.assertEqual(0.73106, np.round(self.function.value(input_data=1), 5))

    def test_should_return_zero_dot_five_value_when_input_is_zero(self) -> None:
        self.assertEqual(0.5, self.function.value(input_data=0))

    def test_should_return_zero_value_when_input_value_is_less_than_zero(self) -> None:
        input_value = self.faker.random.randint(-200, -100)
        self.assertEqual(0, np.round(self.function.value(input_data=input_value), 5))

    def test_should_return_zero_derived_value_when_input_value_is_greater_than_zero(self) -> None:
        input_value = self.faker.random.randint(100, 200)
        self.assertEqual(0.0, np.round(self.function.derived(input_data=input_value), 5))

    def test_should_return_zero_derived_value_when_input_value_is_zero(self) -> None:
        self.assertEqual(0.25, self.function.derived(input_data=0))

    def test_should_return_zero_derived_value_when_input_value_is_less_than_zero(self) -> None:
        input_value = self.faker.random.randint(-200, -100)
        self.assertEqual(0, np.round(self.function.derived(input_data=input_value), 5))

    def test_should_return_values_from_array(self) -> None:
    ...
    def test_should_return_values_derived_from_array(self) -> None:
